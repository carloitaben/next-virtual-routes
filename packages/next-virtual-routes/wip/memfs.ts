// Copied from https://github.com/nounder/effect-memfs
...
export const handleErrnoException =
  ...
  (err: any, [path]: any[]): PlatformError => {
...
export function make(
  ...
  const NFS = memfs.createFsFromVolume(
    contents ? memfs.Volume.fromJSON(contents, cwd) : new memfs.Volume({}),
  )
  ...
  return Effect.map(Effect.serviceOption(FileSystem.WatchBackend), (backend) =>
    FileSystem.make({
      access,
      chmod,
      chown,
...
export const layerWith = (contents: Contents) =>
  Layer.effect(FileSystem.FileSystem, make(contents))
